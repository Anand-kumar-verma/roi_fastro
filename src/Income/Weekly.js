...
const WeeklyIncome = () => {
  ...
  const { isLoading, data } = useQuery(
    ["weekly_income_api", page],
    () =>
      apiConnectorGet(
        `${endpoint?.roi_income_api}?income_type=WEEKLY&page=${page}`
      ),
    {
  ...
  const weekly_data = data?.data?.data || [];
            ...
            <table className="min-w-full ">
              ...
              <tbody className="">
                {weekly_data?.data?.map((item, index) => (
                  <tr key={index}>
                    <td className="md:px-6 text-nowrap px-2 py-4 border border-yellow-500 text-text-color text-center text-colorspace-nowrap">
                      {index + 1}
                    </td>
                    <td className="md:px-6 text-nowrap px-2 py-4 border border-yellow-500 text-text-color text-center text-colorspace-nowrap">
                    ...
                    <td className="md:px-6 text-nowrap px-2 border border-yellow-500 text-text-color text-center py-4 text-colorspace-nowrap">
                      {Number(item?.ledger_amount)}
                    </td>
                    <td className="md:px-6 text-nowrap px-2  border border-yellow-500 text-text-color text-center py-4 text-colorspace-nowrap">
                      {item?.ledger_des}
                    </td>
