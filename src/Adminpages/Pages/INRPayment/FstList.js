...
const FstList = () => {
  ...
  const INRPayoutFunction = async (page) => {
    setloding(true);
    try {
      const res = await axiosInstance.post(API_URLS?.inr_payout_fst_data, {
  ...
  async function handleWithdrawalStatus(t_id, status) {
    setloding(true);
    try {
      const res = await axiosInstance.post(API_URLS?.payout_fst_request_approval, {
  ...
  const tablerow = data?.map((i, index) => {
    return [
      ...
      <span
        className={`${
          i?.wdrl_fst_status === "Success"
            ? "text-green-500"
            : i?.wdrl_fst_status === "Failed"
            ? "!text-rose-500"
            : "!text-gray-800"
        }`}
      >
